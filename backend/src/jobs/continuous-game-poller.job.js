  ...
  try {
    ...
    if (!lastCheckTime) {
      const result = await pool.query(
        `SELECT MAX(created_at) as last_check FROM games WHERE source = 'leaguepedia'`
      );
        ...
        if (newGames && newGames.length > 0) {
          ...
          for (const gameData of newGames) {
            try {
              ...
              totalGamesProcessed++;
              logger.info(`Processed ${region} game: ${gameData.blue_team} vs ${gameData.red_team}`);
            } catch (error) {
              logger.error(`Failed to process ${region} game:`, {
                error: error.message,
                game: `${gameData.blue_team} vs ${gameData.red_team}`
              });
            }
          }
        } else {
